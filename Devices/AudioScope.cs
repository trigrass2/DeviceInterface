...
namespace LabNation.DeviceInterface.Devices {
	...
	public partial class AudioScope : IDevice, IScope {
#if DEBUG
        ...
        private List<short[]> dummy = new List<short[]>();

        public string Serial { get { return "AUDIO"; } }
        public bool Ready { get { return true || latestAudioData != null; } }
        ...
        internal double BASE_SAMPLE_PERIOD = 1f/44100f; //44.1kHz sample rate
        ...
        private void OnAudioJackDataAvailable(object sender, WaveInEventArgs e)
        {
            if (audioJack != null)
            {
                ...
                dummy.Add(sampleData);

                if (dummy.Count > 10)
                {
                    List<short> valList = new List<short>();
                    foreach (var item in dummy)
                    {
                        valList.AddRange(item);
                    }
        ...
        public DataPackageScope GetScopeData()
        {
            ...
            if (acquisitionRunning)
            {
                ...
                while(true)
                {
                    ...
                    float[] wave;
                    //if (latestAudioData == null) return null;
                    lock (this)
                    {
                        //generate 10ms of data, and make it 1 period
                        /*wave = new float[441];                        
                        float amp = r.Next(10);
                        if (amp < 9) amp /= 2f;
                    ...
                    triggerHoldoffInSamples = (int)(TriggerHoldoffCurrent / SamplePeriodCurrent); //OK

                    double triggerTimeout = 0.0;
                    if (AcquisitionModeCurrent == AcquisitionMode.AUTO)
                        triggerTimeout = SamplePeriodCurrent * acquisitionDepthCurrent * 2.0; //Give up after some time

                    //detect whether this section contains a trigger
                    if (logicAnalyserEnabledCurrent && this.triggerValue.mode == TriggerMode.Digital)
                    {
                        triggerDetected = false;// DummyScope.DoTriggerDigital(waveDigital.ToArray(), triggerHoldoffInSamples, digitalTrigger, acquisitionDepthCurrent, out triggerIndex);
                    }
                    else
                    {
                        if (triggerValue.source != TriggerSource.Channel)
                            throw new Exception("Doing analog trigger but mode is not set to analog!");
                        triggerDetected = SoftwareTriggerAnalog(waveAnalog[triggerValue.channel].ToArray(), triggerValue,
                    ...
                    var timePassed = new TimeSpan((long)(wave.Length * SamplePeriodCurrent * 1e7));
                ...
                foreach (AnalogChannel channel in AnalogChannel.List)
                {
                    ...
                    float[] fullData = waveAnalog[channel].ToArray();                    
                    acquisitionBufferAnalog[channel] = DummyScope.CropWave(acquisitionDepthCurrent, fullData, triggerIndex, triggerHoldoffInSamples);
                }
            ...
            while (true)
            {
                viewportDecimation = (int)Math.Ceiling(Math.Log(ViewPortTimeSpan / (samples + 2) / SamplePeriodCurrent, 2)); //affected by decimation
                if (viewportDecimation >= 0)
                    break;
                samples /= 2; //affected by decimation
            }
            ...
            int viewportSamples = (int)(ViewPortTimeSpan / (SamplePeriodCurrent * Math.Pow(2, viewportDecimation))) + 2; //affected by decimation
        ...
        public static bool SoftwareTriggerAnalog(float[] wave, TriggerValue trigger, int holdoff, float threshold, uint width, uint outputWaveLength, out int triggerIndex)
        {
            //Hold off:
            ...
            {
                if (wave[i] < trigger.level && wave[i+1] >= trigger.level)                    
                {
                    int triggerIndexTmp = i;
